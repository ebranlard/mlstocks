...
# --- parameters
TickerList    = './_data/symbol_lists/yahoo_all.csv'
WrongTickFile = '_data/symbol_lists/WrongTicks.csv'
OutFile       = 'Random_Stats.xlsx'
...
# --------------------------------------------------------------------------------}
# --- Reading list and selecting random ticks
# --------------------------------------------------------------------------------{
df = pd.read_csv(TickerList)
# df=df[(df['Exchange']=='NMS') |  (df['Exchange']=='NYQ') |  (df['Exchange']=='PNK')]
ticks=df['Ticker']
print(len(ticks))
bBad=ticks.str.contains('.', regex=False)
ticks=ticks[~bBad]
bBad=ticks.str.len() > 6
ticks=ticks[~bBad]
print(len(ticks))
bBad=ticks.str.contains('^', regex=False)
ticks=ticks[~bBad]
...
#ticks=df['Ticker'].sample(nRandom)
ticks=ticks.sample(nRandom)

df=pd.DataFrame()
df['Ticker']=ticks.values
